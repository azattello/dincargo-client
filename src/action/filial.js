...
export const addFilial = async (filialText, userPhone) => {
  try {
    ...
    const response = await axios.post('https://din-cargo.kz:3001/api/filial/addFilial', {
      filialText,
      userPhone
    });
...
export const getFilials = async () => {
  try {
    ...
    const response = await axios.get('https://din-cargo.kz:3001/api/filial/getFilial');
...
export const fetchFilialByUserPhone = async (userPhone) => {
  try {
      const response = await axios.get('https://din-cargo.kz:3001/api/filial/getFilialByUserPhone', {
          params: {
              userPhone
          }
      });
...
export const deleteFilial = async (filialId) => {
  try {
    const response = await axios.delete(`https://din-cargo.kz:3001/api/filial/deleteFilial/${filialId}`);
